  ...
  if (gmailClientSecret) {
    try {
      ...
      const data = await sendViaGmailApi({
        ...
        html: `<p>Hello ${order.name},</p><p>Thank you for choosing EKG Logistics. We have received your rental request for the <b>${order.productName}</b> (Order #${order.id}).</p><p>Our team will contact you shortly.</p>`,
        attachments: [
          {
  ...
  if (gmailClientSecret) {
    try {
      ...
      const data = await sendViaGmailApi({
        ...
        html: `<p>Hello ${order.name},</p><p>We are pleased to inform you that your rental request (Order #${order.id}) for the <b>${order.productName}</b> has been <b>APPROVED</b>.</p><p>Please find your official invoice attached to this email.</p><p>Our team is now finalizing the logistics for your chauffeur and vehicle.</p><p>Scheduled Dates: ${order.start} to ${order.end}</p><p>You can track your orders here: https://ekgsite.com/my-orders</p><br/><p>Best regards,<br/>EKG Logistics Team</p>`,
        attachments: [
          {
            filename: `invoice-${order.id}.pdf`,
            content: invoiceBuffer
