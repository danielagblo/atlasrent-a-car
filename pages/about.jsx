...
export default function AboutPage() {
  ...
  const [brands, setBrands] = useState(['Toyota', 'Honda', 'Nissan', 'Hyundai'])
    ...
    <Layout>
      ...
      <section className="page page-about" style={{ padding: isMobile ? '20px 20px 40px' : '20px 60px 60px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr', gap: isMobile ? 40 : 60, marginBottom: isMobile ? 60 : 80 }}>
          <motion.div variants={fadeUp} style={{ padding: isMobile ? 32 : 48, background: 'rgba(255,255,255,0.02)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{
              display: 'inline-flex',
              alignItems: 'center',
              borderRadius: 999,
            ...
            <h3 style={{ fontSize: 32, marginBottom: 24, color: '#fff' }}>Driven by Excellence</h3>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              {[
                ...
                <div key={idx} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  ...
                  <p style={{ color: '#fff', margin: 0, fontSize: 15, lineHeight: 1.6 }}>{item}</p>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <h3 style={{ fontSize: isMobile ? 32 : 40, color: '#fff', marginBottom: 16 }}>Our Core Services</h3>
            <p style={{ color: 'var(--muted)', fontSize: 16 }}>End-to-end transport solutions designed for your needs.</p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 24 }}>
            {[
              { icon: '🚗', title: 'Car Rental', desc: 'Ultra-modern cars from Toyota, Honda, Nissan, Hyundai. Short, medium and long-term hire with flexible rates.' },
              ...
              <motion.div key={idx} whileHover={{ y: -8, border: '1px solid rgba(212, 175, 55, 0.4)' }} style={{ padding: 32, background: 'rgba(255,255,255,0.02)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.05)', transition: 'all 0.3s ease' }}>
                ...
                <h4 style={{ marginBottom: 12, fontSize: 20, color: '#fff' }}>{srv.title}</h4>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ marginTop: isMobile ? 60 : 100 }}>
          <h3 style={{ fontSize: isMobile ? 32 : 40, marginBottom: 40, textAlign: 'center', color: '#fff' }}>Chauffeur-Driven Service</h3>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 24 : 40 }}>
            <div style={{ padding: 40, background: 'rgba(255,255,255,0.02)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
                ...
                <h4 style={{ fontSize: 24, color: '#fff', margin: 0 }}>Service Hours</h4>
              </div>
              <div style={{ display: 'grid', gap: 16 }}>
                {[
                  ...
                  <div key={i} style={{ padding: 16, background: 'rgba(0,0,0,0.4)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ fontWeight: 600, marginBottom: 4, color: '#D4AF37', fontSize: 14, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h.k}</div>
                    <div style={{ color: '#fff', fontSize: 15 }}>{h.v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: 40, background: 'rgba(255,255,255,0.02)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
                <div style={{ fontSize: 32 }}>💰</div>
                <h4 style={{ fontSize: 24, color: '#fff', margin: 0 }}>Chauffeur Allowances</h4>
              </div>
              <div style={{ display: 'grid', gap: 16 }}>
                {[
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              {[
                ...
                <div key={idx} style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                  ...
                  <p style={{ color: '#fff', margin: 0, fontSize: 15 }}>{note}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.div whileInView={{ scale: [0.95, 1], opacity: [0, 1] }} transition={{ duration: 0.5 }} viewport={{ once: true }} style={{ marginTop: 80, padding: isMobile ? 40 : 60, background: 'linear-gradient(135deg, rgba(227, 6, 19, 0.1), rgba(212, 175, 55, 0.1))', borderRadius: 24, border: '1px solid rgba(212, 175, 55, 0.2)', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: isMobile ? 28 : 36, marginBottom: 32, color: '#fff' }}>Vehicle Brands We Offer</h3>
          <div style={{ display: 'flex', gap: isMobile ? 24 : 48, justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
            {brands.map(brand => (
              <div key={brand} style={{ fontSize: isMobile ? 24 : 32, fontWeight: 900, color: '#fff', opacity: 0.9, letterSpacing: '-0.02em', margin: '0 12px' }}>{brand}</div>
            ))}
          </div>
